...
import { TEXT, FAINT } from '../../theme.jsx';
      ...
      {items.map(t => {
        ...
        return (
          <div key={t.id} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 6px', borderRadius:'5px', marginBottom:'3px', background:bg }}>
            ...
            <span style={{ fontSize:'11px', color:TEXT, flex:1, minWidth:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.name}</span>
            <span style={{ fontSize:'10px', color:valueColor, fontWeight:'700', flexShrink:0 }}>{getValue(t)}</span>
          </div>
        );
      })}
      {overflow > 0 && <div style={{ fontSize:'10px', color:FAINT, marginTop:'3px', paddingLeft:'6px' }}>+{overflow} more</div>}
    </div>
  );
}